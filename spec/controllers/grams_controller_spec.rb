...
RSpec.describe GramsController, type: :controller do
  ...
  describe "grams#destoy" do
    it "shouldn't allow a user who didn't create the gram to destroy it" do
  ...
  describe "grams#update" do
    ...
    it "should have http 404 error if the gram cannot be found" do
      u = FactoryGirl.create(:user)
      sign_in u
      patch :update, id: "YOLOSWAG", grams: {message: 'Changed'}
      expect(response).to have_http_status(:not_found)
    end

    it "should render the edit form with an http status of unprocessable_entity" do
      p = FactoryGirl.create(:gram, message: "Inital Value")
      sign_in p.user
      patch :update, id: p.id, gram: {message:''}
      expect(response).to have_http_status(:unprocessable_entity)
      p.reload
      expect(p.message).to eq "Inital Value"
    end
  end
  ...
  describe "grams#edit" do
    ...
    it "should successfully show the edit if the gram is found" do
      p = FactoryGirl.create(:gram)
      sign_in p.user
      get :edit, id: p.id
  ...
  describe "grams#show action" do
    it "should successfully show the page if the the gram is found" do
